...
The agent flow to be mirrored is **Start (form input) → Signal Scan LLM Agent → JSON result**, as defined in the provided AgentFlow JSON. fileciteturn4file4
...
Form fields must match the agent flow inputs: fileciteturn4file9
...
- On success, route to `/results/:publicId` and show success toast/banner (“Report generated”).
...
Formatting rules:
- Section headings in `customer_report` must be rendered plainly without “STEP”/step numbers, consistent with the prompt constraints. fileciteturn4file14
...
Public:
- `POST /api/public/scans` → returns `{ publicId }`
- `GET /api/public/results/:publicId` → returns public result model
...
- Public form fields match the agent flow and validate correctly. fileciteturn4file9
- Submission triggers generation and routes to results page.
- Results page displays only customer-safe report. fileciteturn4file14
